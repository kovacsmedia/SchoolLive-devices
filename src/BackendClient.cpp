...
// ---------------------------------------------------------------------------
// fetchSnapPort – tenant Snapcast port lekérése
// ---------------------------------------------------------------------------
uint16_t BackendClient::fetchSnapPort() {
    ...
    if (!ok || code != 200) {
        Serial.printf("[HTTP] fetchSnapPort hiba: %d\n", code);
        return 0;
    }
    uint16_t port = resp["snapPort"] | 0;
    Serial.printf("[HTTP] fetchSnapPort: %d\n", port);
    return port;
}
...
bool BackendClient::getJson(const String& path, JsonDocument& resp, int& httpCode) {
    ...
    if (!http.begin(client, url)) { Serial.println("[HTTP] GET begin() failed"); return false; }
    addCommonHeaders(http);
    httpCode = http.GET();
    Serial.printf("[HTTP] httpCode: %d\n", httpCode);
    if (httpCode <= 0) {
...
bool BackendClient::downloadFile(const String& url, const String& localPath, size_t expectedBytes) {
    if (LittleFS.exists(localPath)) {
        if (expectedBytes == 0) { Serial.printf("[DL] Already exists: %s (skip)\n", localPath.c_str()); return true; }
        File f = LittleFS.open(localPath, "r");
        if (f) { size_t s = f.size(); f.close(); if (s == expectedBytes) return true; }
    }
    waitCooldown();
    String fullUrl = url;
    ...
    uint8_t buf[512]; size_t totalWritten = 0;
    int contentLength = http.getSize();
    unsigned long dlStart = millis();
    while (http.connected() && (contentLength > 0 || contentLength == -1)) {
        size_t avail = stream->available();
        if (avail == 0) { if (millis() - dlStart > 15000) break; delay(1); continue; }
        size_t rd = stream->readBytes(buf, min(avail, sizeof(buf)));
        if (rd > 0) { file.write(buf, rd); totalWritten += rd; dlStart = millis(); }
    ...
    if (expectedBytes > 0 && totalWritten != expectedBytes) { LittleFS.remove(localPath); return false; }
...
bool BackendClient::poll(PolledCommand& outCmd) {
    ...
    JsonDocument req; req["ping"] = (uint32_t)millis();
    JsonDocument resp; int code = 0;
    bool ok = postJson("/devices/poll", req, resp, code);
    if (!ok) return false;
    if (!resp["ok"].is<bool>() || !resp["ok"].as<bool>()) return false;
    if (resp["command"].isNull()) { outCmd.hasCommand = false; return true; }
...
bool BackendClient::confirmProvisioning(const String& provisioningToken,
                                        ...
                                        String& outWifiSsid,
                                        String& outWifiPass) {
    outDeviceKey = ""; outWifiSsid = ""; outWifiPass = "";
