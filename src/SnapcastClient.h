#pragma once
// ─────────────────────────────────────────────────────────────────────────────
// SnapcastClient.h – közvetlen I2S write (ring buffer nélkül)
// ─────────────────────────────────────────────────────────────────────────────
#include <Arduino.h>
#include <WiFi.h>
#include "Config.h"
...
private:
    WiFiClient    _client;
    AudioManager* _audioMgr = nullptr;
    ...
    void sendHello();
    void sendTime(int32_t sec, int32_t usec);
    void sendMessage(uint16_t type, uint16_t refersTo,
                     const uint8_t* payload, uint32_t size);

    void   initI2S();
    void   deinitI2S();
    void   drainToI2S();  // stub – direct write esetén üres

    int64_t nowUs() const;

    // Stub-ok – már nem használjuk, de a .cpp kompatibilitás miatt
